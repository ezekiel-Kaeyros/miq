...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$barplot <- renderPlotly({
      plotly::plot_ly(import_data$data_onreal, x = ~Var1,
                      type = "bar",
                      y = ~percentage,
                      ...
                      hovertext = paste("Person affected: ", import_data$data_onreal$Var1,
                                        "<br>Number of persons :", import_data$data_onreal$Freq,
                                        "<br>Percentage :",import_data$data_onreal$pct1), #) %>%
    ...
    output$piechart <- renderPlotly({
      plot_ly(import_data$data_onreal, labels= ~Var1,
