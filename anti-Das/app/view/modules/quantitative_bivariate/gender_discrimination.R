...
ui <- function(id) {
  ns <- NS(id)
  cards$card_ui("Frequency of Different Forms of Discrimination by Gender",
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    output$barplot <- renderPlotly({
      plotly::plot_ly(import_data$table_gender_disc, x = ~gender, y = ~Freq, color = ~discrimination, type = "bar", colors = yiord_palette,
                      text = ~paste("Gender: ", gender, "<br>Frequency: ", Freq, "<br>Discrimination: ", discrimination)) %>%
    ...
    output$piechart <- renderPlotly({
      gg <- ggplot(import_data$table_gender_disc, aes(gender, discrimination)) +
        geom_tile(aes(fill = Freq)) +
        geom_text(aes(label = round(Freq, 1), text = paste("Gender :", gender, "\nInfluence:", discrimination, "\nCount:", Freq))) +
